...
from schemas.order import OrderCreate, CartItem, OrderUpdate
...
@pytest.mark.asyncio
async def test_get_all_orders_all(session, created_user, created_multiply_products, product_repository, order_repository):
    order_create_data = OrderCreate(user_id=created_user.id, order_products=[
        CartItem(
            product_id=i.id,
...
@pytest.mark.asyncio
async def test_get_all_orders_by_user(session, created_user, created_multiply_products, product_repository, order_repository):
    order_create_data = OrderCreate(user_id=created_user.id, order_products=[
        CartItem(
            product_id=i.id,
            quantity=random.randint(1, 5),
...
@pytest.mark.asyncio
async def test_delete_order(session, created_user, created_product, product_repository, order_repository):
